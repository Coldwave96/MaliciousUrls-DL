...
parser.add_argument('--data.data_dir', type=str, default='IntegratedData/test.csv', metavar="DATADIR",
                    help="location of data file")
parser.add_argument('--data.subword_dict_dir', type=str, default="Model/runs/emb1_32dim_minwf1_1conv3456_5ep/subwords_dict.pickle", 
		            metavar="SUBWORD_DICT", help="directory of the subword dictionary")
parser.add_argument('--data.word_dict_dir', type=str, default="Model/runs/emb1_32dim_minwf1_1conv3456_5ep/words_dict.pickle", 
		            metavar="WORD_DICT", help="directory of the word dictionary")
parser.add_argument('--data.char_dict_dir', type=str, default="Model/runs/emb1_32dim_minwf1_1conv3456_5ep/chars_dict.pickle", 
		            metavar="CHAR_DICT", help="directory of the character dictionary")
...
parser.add_argument('--log.checkpoint_dir', type=str, default="Model/runs/emb1_32dim_minwf1_1conv3456_5ep/checkpoints/", metavar="CHECKPOINTDIR",
...
urls, labels = utils.load_test_data(FLAGS["data.data_dir"])
...
ngramed_id_x, worded_id_x = utils.ngram_id_x_from_dict(word_x, FLAGS["data.max_len_subwords"], ngram_dict, word_dict) 
...
with graph.as_default(): 
    ...
    with sess.as_default(): 
        ...
        if FLAGS["model.emb_mode"] == 1: 
            batches = utils.batch_iter(list(chared_id_x), FLAGS["test.batch_size"], 1, shuffle=False) 
        elif FLAGS["model.emb_mode"] == 2: 
            batches = utils.batch_iter(list(worded_id_x), FLAGS["test.batch_size"], 1, shuffle=False) 
        elif FLAGS["model.emb_mode"] == 3: 
            batches = utils.batch_iter(list(zip(chared_id_x, worded_id_x)), FLAGS["test.batch_size"], 1, shuffle=False)
        elif FLAGS["model.emb_mode"] == 4: 
            batches = utils.batch_iter(list(zip(ngramed_id_x, worded_id_x)), FLAGS["test.batch_size"], 1, shuffle=False)
        elif FLAGS["model.emb_mode"] == 5: 
            batches = utils.batch_iter(list(zip(ngramed_id_x, worded_id_x, chared_id_x)), FLAGS["test.batch_size"], 1, shuffle=False)    
        all_predictions = []
        all_scores = []
        ...
        #for batch in batches:
            ...
            if FLAGS["model.emb_mode"] in [4, 5]:
                x_char, x_char_pad_idx = utils.pad_seq(x_char, FLAGS["data.max_len_words"], FLAGS["data.max_len_subwords"], FLAGS["model.emb_dim"])
...
utils.save_test_result(labels, all_predictions, all_scores, FLAGS["log.output_dir"])
