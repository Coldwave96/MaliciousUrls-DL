...
def get_vocabulary(urls, max_length, min_freq=0):
    # Create the VocabularyProcessor object, setting the max length of the urls
    processor = tflearn.data_utils.VocabularyProcessor(max_length, min_freq)

    # Transform the urls using the vocabulary
    start = time.time()
    x = np.array(list(processor.fit_transform(urls)))
    print("Finished build vocabulary and mapping to x in {}".format(time.time() - start))

    # Extract word:id mapping from the object
    vocabulary_dict = processor.vocabulary_._mapping

    # Treat the id's as index into dict and create a dict of words
    reverse_dict = dict(zip(vocabulary_dict.values(), vocabulary_dict.keys()))
...
def get_words(x, reverse_dict, delimit_mode, urls=None):
    processed_x = []
    # Delimit URL by special characters (i.e. non-alphanumerical characters)
    if delimit_mode == 0:
        for url in x:
            words = []
            for word_id in url:
    ...
    # Delimit URL by special characters (i.e. non-alphanumerical characters)
    # Treat each special characters as words
    elif delimit_mode == 1:
        for i in range(x.shape[0]):
            word_url = x[i]
            raw_url = urls[i]
            words = []
            for j in range(len(word_url)):
                word_id = word_url[j]
                if word_id == 0:
...
def char_id_x(urls, char_dict, max_len_chars):
    ...
    for url in urls:
        url = list(url)
        url_in_char_id = []
        length = min(len(url), max_len_chars)
        ...
        processed_char_id_x.append(url_in_char_id)
    return processed_char_id_x
