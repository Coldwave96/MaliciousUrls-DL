...
class URLNet(object):
    def __init__(self, char_ngram_vocab_size, word_ngram_vocab_size, char_vocab_size, word_seq_len, char_seq_len,
                 embedding_size, l2_reg_lambda=0, filter_sizes=None, mode=0):
        if filter_sizes is None:
            filter_sizes = [3, 4, 5, 6]
        if mode == 4 or mode == 5:
            self.input_x_char = tf.compat.v1.placeholder(tf.int32, [None, None, None], name="input_x_char")
            self.input_x_char_pad_idx = tf.compat.v1.placeholder(tf.float32, [None, None, None, embedding_size],
        ...
        self.input_y = tf.compat.v1.placeholder(tf.float32, [None, 2], name="input_y")
        ...
        with tf.name_scope("output"):
            w0 = tf.compat.v1.get_variable("w0", shape=[1024, 512], initializer=tf.keras.initializers.glorot_normal())
            ...
            w1 = tf.compat.v1.get_variable("w1", shape=[512, 256], initializer=tf.keras.initializers.glorot_normal())
            b1 = tf.Variable(tf.constant(0.1, shape=[256]), name="b1")
            l2_loss += tf.nn.l2_loss(w1)
            l2_loss += tf.nn.l2_loss(b1)
            output1 = tf.nn.relu(tf.matmul(output0, w1) + b1)

            w2 = tf.compat.v1.get_variable("w2", shape=[256, 128], initializer=tf.keras.initializers.glorot_normal())
            b2 = tf.Variable(tf.constant(0.1, shape=[128]), name="b2")
            l2_loss += tf.nn.l2_loss(w2)
            l2_loss += tf.nn.l2_loss(b2)
            output2 = tf.nn.relu(tf.matmul(output1, w2) + b2)

            w = tf.compat.v1.get_variable("w", shape=(128, 2), initializer=tf.keras.initializers.glorot_normal())
